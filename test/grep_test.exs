defmodule Unused.GrepTest do
  ...
  describe "find/2" do
    ...
    test "returns found for all terms in files" do
      assert %{
               tag_a: %{@text1 => [{@file_name2, [@text1]}, {@file_name, [@text1]}]},
               tag_b: %{@text2 => [{@file_name2, [@text2]}, {@file_name, [@text2]}]}
             } == Grep.find("test_file*", [pattern(:tag_a, @text1), pattern(:tag_b, @text2)])
    end
  end

  test "format_results" do
    results = [
      [{:templates, "search1", "file1", []}],
      [{:templates, "search1", "file2", []}],
      [{:templates, "search1", "file3", ["found"]}]
    ]

    assert %{
             templates: %{
               "search1" => [{"file1", []}, {"file2", []}, {"file3", ["found"]}]
             }
           } == Grep.format_results(results)
  end
end
