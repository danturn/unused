defmodule Unused.Templates do
  ...
  def get(project_path) do
    ...
    rootnames = Enum.map(unused_templates, fn x -> {:templates, x, ~r|\b#{Path.basename(x, ".html.eex")}\b|} end)
  ...
  def not_found_only(list) do
    list
    |> Enum.flat_map(fn {_, files} -> Enum.reject(files, fn {_, occurences} -> Enum.any?(occurences, fn {_, occurence} -> occurence != [] end) end) end)
